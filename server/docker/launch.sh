...
if [ -f "$UPDATES/pending" ]; then
  ...
  if [ -n "$want" ] && [ -x "$UPDATES/$want/bin/server" ]; then
    say "starting update $want"
    echo "$want" > "$UPDATES/trying"
    exec "$UPDATES/$want/bin/server" "$@"
  fi
...
if [ -f "$UPDATES/active" ]; then
  ...
  if [ -n "$have" ] && [ -x "$UPDATES/$have/bin/server" ]; then
    exec "$UPDATES/$have/bin/server" "$@"
  fi
